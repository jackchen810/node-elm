...
async function updateDatabase(data_s, symbol) {
    ...
    await data_s.to_download('day', 'fq', symbol,  function(record){
        ...
        for (var i = 0; i < data.length; i++) {
            ...
            //参数检查
            DB.KHistory('day', symbol).findOne(wherestr).exec(function (err, doc ) {
                if (doc == null) {
                    DB.KHistory('day', symbol).create(updatestr);
                }
            });
        }
    });
