...
class ScriptFileHandle {
    ...
    async all_file(req, res, next){
        ...
        console.log('[http] file list end');
    }

    async all_doc(req, res, next){
        console.log('[http] all doc list');
        ...
        console.log('[http] all doc list end');
    }

    async file_list(req, res, next){
        console.log('[http] script file list');
        //获取表单数据，josn
        ...
        console.log('[http] script filter:', filter);
        ...
        if(typeof(page_size)==="undefined" && typeof(current_page)==="undefined"){
            ...
            res.send({ret_code: 0, ret_msg: '成功', extra:fileList, total:total});
        }
        ...
        console.log('[http] script file list end');
    }


    async list(req, res, next){
        console.log('[http] script list');
        //获取表单数据，josn
        var page_size = req.body['page_size'];
        var current_page = req.body['current_page'];
        var sort = req.body['sort'];
        ...
        //console.log('sort ', sort);
        console.log('[http] script files filter:', filter);
        var total = await DB.ScriptFileTable.count(filter).exec();
        ...
        console.log('[http] script list end');
    }


    async del(req, res, next){
        console.log('[http] script del');
        //获取表单数据，josn
        ...
        console.log('[http] script del end');
    }


    async download(req, res, next) {
        console.log('[http] script download');
        ...
        console.log('[http] script download end');
    }
    ...
    get_dest_fullname(file_type){
        ...
        if (file_type == 'select'){
            ...
            basePath = config.order_channel_dir;
        }
        else if(file_type == 'market'){
            basePath = config.market_channel_dir;
        }
        return basePath;
    }
    ...
    async upload(req, res, next){
        ...
        console.log('[http] script upload');
        //console.log(req);
        ...
        */
        ...
        form.parse(req);
        console.log('[http] script upload end');

    }

    async revoke(req, res, next){
        console.log('[http] script revoke');
        ...
        console.log('[http] script revoke end');

    }


    async release(req, res, next) {
        console.log('[http] script release');
        ...
        var query = await DB.ScriptFileTable.findByIdAndUpdate(id, updatestr);
        res.send({ret_code: 0, ret_msg: 'SUCCESS', extra: query});
        console.log('[http] script release end');

    }

    async sync_file(user_account, path) {
        console.log('[http] script sync_file');
        ...
        else if(path == config.order_channel_dir){
            file_type = 'order';
        }
        else if(path == config.market_channel_dir){
            file_type = 'market';
        }

        console.log('sync_file user_account: ', user_account);
        ...
        console.log('[http] script sync_file end');
    }

    async sync(req, res, next) {
        console.log('[http] script sync');
        var user_account = req.session.user_account;

        //console.log('sync user_account: ', req.session);
        ...
        await this.sync_file(user_account, config.order_channel_dir);
        await this.sync_file(user_account, config.market_channel_dir);

        res.send({ret_code: 0, ret_msg: '成功', extra: ''});
        console.log('[http] script sync end');
    }
}
