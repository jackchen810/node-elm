...
class MonitorHandle {
    ...
    async add(req, res, next) {
        console.log('[http] simulate task add');
        ...
        //发送任务
        HttpTx.send(message, 'trade.task', 'add', ['trader', 'gateway']);
        HttpRx.addOnceListener(task_id, async function(type, action, response) {
            ...
            if (response['ret_code'] == 0) {
                ...
                console.log('[http] simulate task add end');
            }
            else{
    ...
    async del(req, res, next) {
        console.log('[http] simulate task del');
        ...
        //发送任务,trader 删除任务
        HttpTx.send(queryList, 'trade.task', 'del', ['trader', 'gateway']);
        HttpRx.addOnceListener(task_id, async function(type, action, response) {
            //console.log('del task, response', response);
            if (response['ret_code'] == 0) {
                await DB.TaskTable.remove(wherestr).exec();
                res.send({ret_code: 0, ret_msg: 'SUCCESS', extra: task_id});
                console.log('[http] simulate task del end');
            }
            else{
    ...
    async start(req, res, next) {
        console.log('[http] simulate task start');
        ...
        HttpRx.addOnceListener(task_id, async function(type, action, response) {
            ...
            if (response['ret_code'] == 0) {
                ...
                res.send({ret_code: 0, ret_msg: 'SUCCESS', extra: task_id});
                console.log('[http] simulate task start end');
            }
            else{
                console.log('error:', response['extra']);
                res.send(response);
    ...
    async stop(req, res, next) {
        console.log('[http] simulate task stop');
        ...
        HttpRx.addOnceListener(task_id, async function(type, action, response) {
            ...
            if (response['ret_code'] == 0) {
                ...
                await DB.TaskTable.update(wherestr, updatestr).exec();
                res.send({ret_code: 0, ret_msg: 'SUCCESS', extra: task_id});
                console.log('[http] simulate task stop end');
            }
            else{
                console.log('error:', response['extra']);
                res.send(response);
