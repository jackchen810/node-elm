...
class PickStockStrategyHandle {
    constructor(){
        //绑定，this
        this.strategy_list = this.strategy_list.bind(this);
    ...
    async strategy_list(req, res, next){
        ...
        var total = await DB.PickStrategyTable.count(filter).exec();

        //参数有效性检查
        if(typeof(page_size)==="undefined" && typeof(current_page)==="undefined"){
            var queryList = await DB.PickStrategyTable.find(filter).sort(sort);
            res.send({ret_code: 0, ret_msg: 'SUCCESS', extra:queryList, total:total});
        }
        else if (page_size > 0 && current_page > 0) {
            var skipnum = (current_page - 1) * page_size;   //跳过数
            var queryList = await DB.PickStrategyTable.find(filter).sort(sort).skip(skipnum).limit(page_size);
            res.send({ret_code: 0, ret_msg: 'SUCCESS', extra:queryList, total:total});
        }
        else{
    ...
    async del(req, res, next){
        ...
        await DB.PickStrategyTable.remove(wherestr).exec();

        res.send({ret_code: 0, ret_msg: 'SUCCESS', extra:file_name});
        console.log('pick strategy del end');
    }


    async download(req, res, next) {
        console.log('[website] pickstock download');
        ...
        //检查上下架状态
        try {
            var query = await DB.PickStrategyTable.findById(id).exec();
        ...
        console.log('[website] pickstock download end');
    }
    ...
    async upload(req, res){
        ...
        console.log('[website] pickstock upload');
        //console.log(req);
        ...
        */
        ...
        form.on('end', async function () {
            ...
            var query = await DB.PickStrategyTable.findOne({'file_name': fileName}).exec();
            if (query != null){
            ...
            fs.rename(uploadedPath, dstPath, function(err) {
                if(err){
                    ...
                    DB.PickStrategyTable.create(romDocObj);
                }
            });
        });
        ...
        form.parse(req);
        console.log('[website] pickstock upload end');

    }

    async revoke(req, res, next){
        console.log('[website] pickstock revoke');
        ...
        var query = await DB.PickStrategyTable.findByIdAndUpdate(id, updatestr);
        res.send({ret_code: 0, ret_msg: 'SUCCESS', extra: query});
        console.log('[website] pickstock revoke end');

    }


    async release(req, res, next) {
        console.log('[website] pickstock release');
        ...
        var query = await DB.PickStrategyTable.findByIdAndUpdate(id, updatestr);
        res.send({ret_code: 0, ret_msg: 'SUCCESS', extra: query});
        console.log('[website] pickstock release end');

    }

}

module.exports = new PickStockStrategyHandle()
